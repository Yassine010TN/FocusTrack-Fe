...
export default function GoalDetailsPage() {
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8080/api/sharing/shared/${goalId}/users`, {
      headers: { Authorization: `Bearer ${token}` },
    })
  ...
  const handleShare = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:8080/api/sharing/share?goalId=${goalId}&contactId=${selectedContactId}`,
        {
          method: "POST",
          headers: {
            Authorization: `Bearer ${token}`,
      ...
      const refreshed = await fetch(`http://localhost:8080/api/sharing/shared/${goalId}/users`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      if (refreshed.ok) {
